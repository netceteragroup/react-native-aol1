...
//  ADTECHMobileSDK
//
//  Created by ADTECH GmbH on 5/10/12.
 ...
 * The property is readonly and its value is loaded from the *ADTECHAdConfiguration.plist* when the corresponding ADTECH object (ATBannerView, ATInterstitial, ATMoviePlayerController) is created.
 ...
 * By changing the value at runtime you can configure different schemes for different ADTECH objects (ATBannerView, ATInterstitial, ATMoviePlayerController).
 *
 ...
 * The default value will be loaded from the *ADTECHAdConfiguration.plist* property file when needed and you can change this value anytime at runtime.
 * By changing the value at runtime you can configure different domains for different ADTECH objects (ATBannerView, ATInterstitial, ATMoviePlayerController).
 *
 * Available in 1.0 and later.
 */
...
/**
 * Returns the build number of the SDK in use.
 *
 * @since 3.8
 */
+ (NSString*)build;
...
@interface ATBaseConfiguration(Location)

/**
 Returns the active location reporting mode.

 @see setLocationReportingMode:
 @since 3.5.2
 */
+ (ATLocationReportingMode)locationReportingMode;

/**
 
 Call this method to control the SDK sending the device's location along with requests for ads
 for the placements that have geo-fencing enabled.
 
 @param mode The way the SDK should handle location reporting. See ATLocationReportingMode for possible values.
 
 @discussion The SDK allows the app to control the way location reporting is performed. By default, location reporting is disabled.
 If it wants the SDK to send out the location, the app can use two modes of operation:
 
 1. SDK - The SDK creates its own CLLocationManager instance and starts monitoring significant location changes.
 Use the exposed CLLocationManager instance to fine-tune location reporting to best suit your needs by calling sharedLocationManager.
 
 2. App - The app is responsible for providing the location to the SDK. It can do that by calling updateLocation:
 
 @see updateLocation:
 @see sharedLocationManager
 
 @warning You need to get permission from the user to allow sending the location
 for ad targeting purposes before calling this method to enable reporting.
 
 @note Location reporting needs to be enabled server-side as well for locations to be sent out by the SDK.
 
 @since 3.5.2
 
 */
+ (void)setLocationReportingMode:(ATLocationReportingMode)mode;

/**
 Call this method providing it the location you want sent with the requests when the reporting is in App mode.
 
 @param location The location fed into the SDK to be sent out with requests
 
 @note Locations older than 1 day are not used.
...
/**
 ...
 Returns the shared location manager instance use by the SDK for location reporting.
 
 After enabling location reporting, by calling setLocationReportingMode: with LocationReportingSDK, use the returned instance
 to fine tune the manager's behavior.
 
 @see setLocationReportingMode:
 
 @warning When in SDK mode, stoping location updates and/or changing the location manager's delegate will break the SDK's
 location reporting.
