...
/**
 * The companion ads that come with a linear ad. You can get the right companion
 ...
 * @since 3.8.1
 */
extern const NSString *kATCompanions;

/**
 * The number of seconds from the beginning of the linear ad when it will become skippable.
 *
 * @since 3.8.1
 */
extern const NSString *kATVASTSkipTime;
...
/**
 * Called when the player is about to start playback of a linear ad. It also has additional information such as
 * the companion ads that come with a linear ad.
 *
 * @param type The linear ad type
 * @param metaData Aditional information that pretains to the linear ad. See keys kATCompanions and kATVASTSkipTime for
 *                 possible values.
 *
 * @discussion This callback has priority over playerWillStartPlayingLinearAdOfType: and playerWillStartPlayingLinearAdOfType:withCompanions:. 
 *             If any of them are implemented, only this callback will be called.
 *
 *             You are responsible for holding on to the companion ad views, by either adding them to your view hierarchy
 *             or creating a strong reference to them for later use. If you don't hold on to these references, the SDK
 *             will release them after returning from this callback.
 ...
 * @since 3.8.1
 */
- (void)playerWillStartPlayingLinearAdOfType:(ATVideoAdType)type withMetaData:(NSDictionary*)metaData;
